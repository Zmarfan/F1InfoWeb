...
class FetchDriverStandingsTaskTest {
    ...
    @Test
    void should_not_call_ergast_for_driver_standings_data_if_there_is_no_next_race_to_fetch_for() throws SQLException, NoDataAvailableYetException {
    ...
    @Test
    void should_not_attempt_to_merge_in_driver_standings_data_to_database_if_no_data_was_returned_from_ergast() throws SQLException, NoDataAvailableYetException {
    ...
    @Test
    void should_merge_in_driver_standings_data_sent_from_ergast_to_database() throws SQLException, ParseException, MalformedURLException, NoDataAvailableYetException {
        when(mDatabase.getNextRaceToFetchDriverStandingsFor()).thenReturn(Optional.of(RACE_RECORD));
    ...
    @Test
    void should_log_severe_if_exception_is_thrown() throws SQLException, ParseException, MalformedURLException, NoDataAvailableYetException {
        when(mDatabase.getNextRaceToFetchDriverStandingsFor()).thenReturn(Optional.of(RACE_RECORD));
        when(mErgastProxy.fetchDriverStandingsForRace(RACE_RECORD)).thenReturn(getDriverStandingsData());
    ...
    @Test
    void should_set_last_fetched_race_after_merging_driver_standings() throws SQLException, ParseException, MalformedURLException, NoDataAvailableYetException {
        when(mDatabase.getNextRaceToFetchDriverStandingsFor()).thenReturn(Optional.of(RACE_RECORD));
        when(mErgastProxy.fetchDriverStandingsForRace(RACE_RECORD)).thenReturn(getDriverStandingsData());
    ...
    @Test
    void should_not_set_last_fetched_race_after_merging_driver_standings_if_it_throws() throws SQLException, ParseException, MalformedURLException, NoDataAvailableYetException {
        when(mDatabase.getNextRaceToFetchDriverStandingsFor()).thenReturn(Optional.of(RACE_RECORD));
        when(mErgastProxy.fetchDriverStandingsForRace(RACE_RECORD)).thenReturn(getDriverStandingsData());
        doThrow(new SQLException("error")).when(mDatabase).mergeIntoDriverStandingsData(anyList(), eq(RACE_RECORD));
