...
public class UserSettingsValidator {
    static final int MAX_DISPLAY_NAME_LENGTH = 20;

    public static boolean newUserSettingsAreOfValidFormat(final NewUserSettingsRequestBody newSettings) {
        return newSettings.getDisplayName() != null
            && newSettings.getDisplayName().trim().length() > 0
            && newSettings.getDisplayName().length() <= MAX_DISPLAY_NAME_LENGTH;
    }
