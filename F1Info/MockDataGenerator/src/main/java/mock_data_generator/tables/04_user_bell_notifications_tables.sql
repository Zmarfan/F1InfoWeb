...
create table bell_notification_click_types(
  type varchar(30) not null,

  constraint bell_notification_click_types_pk primary key (type)
);
...
create table bell_notifications(
  ...
  constraint bell_notifications_click_type_fk foreign key (click_type) references bell_notification_click_types(type)
);
